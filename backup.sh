...
cd /root
zip -r $IP-$date-$domain-parry.zip backup > /dev/null 2>&1
rclone copy /root/$IP-$date-$domain-parry.zip dr:backup/
url=$(rclone link dr:backup/$IP-$date-$domain-parry.zip)
...
rm -rf /root/backup
rm -r /root/$IP-$date-$domain-parry.zip
echo ""
